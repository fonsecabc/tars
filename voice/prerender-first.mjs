...
for (const text of ALL_FIRST_PHRASES) {
  try {
    const res = await fetch(SPEAK_URL, {
      ...
      body: JSON.stringify({ text, raw: true, render: true }),
      signal: AbortSignal.timeout(30000),
    });
    const r = await res.json().catch(() => ({}));
    console.log(
      `${r.cached ? (r.already ? 'exists ' : 'baked  ') : 'FAILED '} ${JSON.stringify(text)}${r.error ? ' — ' + r.error : ''}`,
    );
  } catch (e) {
    console.log(`FAILED  ${JSON.stringify(text)} — ${e.message}`);
  }
}
