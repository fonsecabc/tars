...
const BACKEND =
  OPENROUTER_KEY && process.env.TARS_GEM_BACKEND !== 'ollama' ? 'openrouter' : 'ollama';
...
async function sessions() {
  ...
  try {
    ...
    sessCache = { at: Date.now(), snap, text: describeSessions(snap) };
  } catch (e) {
    log('sessions failed:', e.message);
  }
  return sessCache;
}
...
function setFocus(s) {
  try {
    writeFileSync(
      FOCUS_FILE,
      JSON.stringify({ label: s.project, sessionId: s.sessionId, cwd: s.cwd, at: Date.now() }),
    );
  } catch {
    /* ignore */
  }
...
function resolveTarget(snap, query, focus) {
  if (query) {
    const m = matchSession(snap, query);
    if (m) return m;
  }
  if (focus) {
    const f = snap.find((s) => s.sessionId === focus.sessionId || s.project === focus.label);
    if (f) return f;
  }
  return null;
}
...
function shape(parsed) {
  ...
  const action = ACTIONS.includes(p.action)
    ? p.action
    : p.talk_to_claude
      ? 'send_to_claude'
      : 'answer';
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', () => {
      ...
      try {
        ...
        if (data?.error)
          return finish(() => reject(new Error(JSON.stringify(data.error).slice(0, 160))));
        ...
        finish(() => reject(new Error(`parse: ${e.message} :: ${out.slice(0, 120)}`)));
      }
    });
...
async function callOllama(system, messages, maxTokens, json) {
  const body = {
    model: OLLAMA_MODEL,
    stream: false,
    options: { temperature: 0.3, num_ctx: 4096, num_predict: maxTokens },
...
const call = (system, messages, { maxTokens = 512, json = false } = {}) =>
  BACKEND === 'openrouter'
    ? callOpenRouter(system, messages, maxTokens)
    : callOllama(system, messages, maxTokens, json);
...
async function summarize(rawReply) {
  ...
  try {
    return (
      await call(
        SUMMARY_SYSTEM,
        [{ role: 'user', content: `The session's latest reply:\n\n${rawReply.slice(0, 4000)}` }],
        { maxTokens: 200 },
      )
    ).trim();
  } catch (e) {
...
async function notify({ text, topic, cwd, session_id }) {
  ...
  const target =
    snap.find((s) => s.sessionId === session_id) || (topic && matchSession(snap, topic)) || null;
...
async function speak(text, { cache = false } = {}) {
  ...
  try {
    await fetch(SPEAK_URL, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ text, raw: true, priority: 'interactive', source: 'router', cache }),
...
async function inject(text, target) {
  try {
    const res = await fetch(INJECT_URL, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ text, submit: true, target }),
...
async function hands(op, arg) {
  try {
    const res = await fetch(HANDS_DO_URL, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
...
const RISKY_ARG =
  /\b(delete|remove|\brm\b|kill|shutdown|reboot|restart|erase|format|uninstall|drop|overwrite|--force|-f\b|sudo|close all|quit all)\b/i;
const isRisky = (op, arg) => op === 'run' || RISKY_ARG.test(String(arg || ''));
const AFFIRM =
  /\b(yes|yeah|yep|yup|do it|go|go ahead|confirm|confirmed|proceed|send it|sure|ok|okay|affirmative)\b/i;
const CONFIRM_FILE = join(os.homedir(), '.tars', 'awaiting-confirm');
let pending = null; // { op, arg, at }
function setAwaiting(on) {
  try {
    if (on) writeFileSync(CONFIRM_FILE, String(Date.now()));
    else unlinkSync(CONFIRM_FILE);
  } catch {
    /* ignore */
  }
}
...
async function runComputer(op, arg) {
  ...
  if (!r.ok) {
    if (r.error === 'no-accessibility')
      return "I can't control apps yet. Grant TarsHands accessibility.";
    if (r.error === 'screencapture')
      return "I can't see the screen yet. Grant TarsHands screen recording.";
  ...
  if (op === 'run')
    return r.stdout
      ? `Done. ${String(r.stdout).replace(/\s+/g, ' ').trim().slice(0, 160)}`
      : 'Done.';
  return 'Done.';
}
...
async function describeScreen(question) {
  ...
  if (!shot.ok)
    return shot.error === 'screencapture'
      ? "I can't see the screen yet. Grant TarsHands screen recording."
      : "I couldn't grab the screen.";
  if (BACKEND !== 'openrouter') return 'Screen vision needs the cloud model.';
  let b64;
  try {
    b64 = readFileSync(shot.path).toString('base64');
  } catch {
    return "I couldn't read the screenshot.";
  }
  const messages = [
    {
      role: 'user',
  ...
  try {
    const raw = await callOpenRouter(SYSTEM_VISION, messages, 220);
    return String(raw || '').trim() || "I looked, but I've got nothing useful to say.";
  } catch (e) {
...
async function hear(text, addressed) {
  ...
  if (pending && Date.now() - pending.at < 30000) {
    ...
    if (AFFIRM.test(clean)) {
      ...
      pushHistory('assistant', `(confirmed) ${say}`);
      return { ok: true, confirmed: true, op: p.op };
    }
    await speak('Cancelled.');
    pushHistory('user', clean);
    pushHistory('assistant', 'Cancelled.');
    return { ok: true, cancelled: true };
  }
  if (pending) {
    pending = null;
    setAwaiting(false);
  } // stale confirmation — drop it
  ...
  let d;
  try {
    d = await gem(clean, sessText, addressed, focus);
  } catch (e) {
    log('gem failed:', e.message);
    if (addressed) await speak('My model is not answering.');
    return { ok: false, error: e.message };
  }
  log(
    `heard(addr=${addressed}) ${JSON.stringify(clean)} -> action=${d.action} session=${JSON.stringify(d.session)} reply=${JSON.stringify(d.reply)}`,
  );
  ...
  switch (d.action) {
    case 'focus_session': {
      ...
      if (!target) {
        note = `I don't see a ${d.session || 'matching'} session.`;
        await speak(note);
        result = { ok: true, ...d };
        break;
      }
    ...
    case 'read_session': {
      ...
      if (!target) {
        note = `I don't see a ${d.session || 'focused'} session to read.`;
        await speak(note);
        result = { ok: true, ...d };
        break;
      }
      ...
      const line = gist
        ? `On ${target.project}: ${gist}`
        : `${target.project} hasn't said anything yet.`;
    ...
    case 'send_to_claude': {
      ...
      if (!willInject) {
        ...
        await speak(note);
        result = { ok: true, ...d, injected: false };
        break;
      }
      ...
      const target = resolveTarget(snap, d.session, focus);
      const tgt =
        target && target.host === 'terminal' && target.tty
          ? { host: 'terminal', tty: target.tty, app: target.app }
          : null;
      ...
      if (!r.ok && r.error === 'tty-not-found')
        await speak("I couldn't find that terminal. Bring it forward and try again.");
      else if (!r.ok && r.error === 'no-accessibility')
        await speak("I can't type yet. Grant TarsHands accessibility in settings.");
    ...
    case 'computer': {
      if (!d.op) {
        await speak(d.reply || "I didn't catch what to do.");
        result = { ok: true, ...d };
        break;
      }
      // Mutating ops need the wake word; 'see' is read-only so it's allowed either way.
      if (d.op !== 'see' && !addressed) {
        note = "Say 'TARS' and I'll do that.";
        await speak(note);
        result = { ok: true, ...d };
        break;
      }
      if (isRisky(d.op, d.arg)) {
        ...
        await speak(
          d.op === 'run'
            ? `About to run that command. Say confirm, or cancel.`
            : `About to ${d.arg}. Confirm, or cancel.`,
        );
...
server.listen(PORT, '127.0.0.1', () =>
  log(`tars-router on :${PORT} · backend=${BACKEND} · gem=${MODEL}`),
);
