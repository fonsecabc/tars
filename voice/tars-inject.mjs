...
async function inject(text, submit, target) {
  ...
  try {
    if (target && target.host === 'terminal' && target.tty && !/iterm/i.test(target.app || '')) {
      ...
      await execFileP('/usr/bin/osascript', ['-e', 'tell application "iTerm" to activate'], {
        timeout: 6000,
      });
    ...
    if (!result) {
      log(`inject${submit ? '' : ' [dry]'} -> ${landed}: ${JSON.stringify(clean).slice(0, 100)}`);
      result = { ok: true, submitted: !!submit, landed };
    }
  } catch (e) {
    ...
    result = {
      ok: false,
      error: /1002|not allowed/i.test(detail) ? 'no-accessibility' : 'osascript',
      detail,
    };
  } finally {
...
async function opApp(name) {
  // open / bring an app to the front
  if (!name) return { ok: false, error: 'no app' };
  await execFileP('/usr/bin/open', ['-a', String(name)], { timeout: 8000 });
  return { ok: true, opened: name };
}
async function opRun(command) {
  // arbitrary shell — RISKY (confirm upstream)
  if (!command) return { ok: false, error: 'no command' };
  try {
    const { stdout, stderr } = await execFileP('/bin/zsh', ['-lc', String(command)], {
      timeout: 60000,
      maxBuffer: 4 << 20,
    });
    return {
    ...
    return { ok: false, error: 'shell', detail: String(e.stderr || e.message || e).slice(0, 800) };
  }
}
async function opSee() {
  // screenshot the screen (needs Screen Recording grant)
  try {
    await execFileP('/usr/sbin/screencapture', ['-x', SEE_PATH], { timeout: 8000 });
    return { ok: true, path: SEE_PATH };
  } catch (e) {
    return { ok: false, error: 'screencapture', detail: String(e.message || e).slice(0, 200) };
  }
}
async function opType(text) {
  // paste text into whatever app is frontmost
  ...
  try {
    ...
    return {
      ok: false,
      error: /1002|not allowed/i.test(String(e.stderr || e)) ? 'no-accessibility' : 'osascript',
    };
  } finally {
    if (prev !== null) setTimeout(() => pbcopy(prev).catch(() => {}), 300);
  }
...
async function opKey(arg) {
  // volume / mute / media / arbitrary combo
  const a = String(arg || '')
    .toLowerCase()
    .trim();
  try {
    if (/\bunmute\b/.test(a)) {
      ...
      return { ok: true, did: 'unmute' };
    }
    if (/\bmute\b/.test(a)) {
      await osa('set volume with output muted');
      return { ok: true, did: 'mute' };
    }
    const setv = a.match(/volume (?:to )?(\d{1,3})/);
    if (setv) {
      await osa(`set volume output volume ${Math.min(100, +setv[1])}`);
      return { ok: true, did: `volume ${setv[1]}` };
    }
    if (/volume up|louder/.test(a)) {
      await osa('set volume output volume (output volume of (get volume settings) + 15)');
      return { ok: true, did: 'volume up' };
    }
    if (/volume down|quieter|lower/.test(a)) {
      await osa('set volume output volume (output volume of (get volume settings) - 15)');
      return { ok: true, did: 'volume down' };
    }
    if (/play|pause|next|previous|skip/.test(a)) {
      const app = /spotify/.test(a) ? 'Spotify' : 'Music';
      const cmd = /next|skip/.test(a)
        ? 'next track'
        : /previous/.test(a)
          ? 'previous track'
          : 'playpause';
      await osa(`tell application "${app}" to ${cmd}`);
      return { ok: true, did: `${app} ${cmd}` };
    }
    ...
    return {
      ok: false,
      error: 'osascript',
      detail: String(e.stderr || e.message || e).slice(0, 200),
    };
  }
}
...
if (argv.length && !argv.includes('--serve')) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === 'POST' && req.url === '/do') {
      ...
      log(
        `do ${body.op} ${JSON.stringify(body.arg || '').slice(0, 80)} -> ${JSON.stringify(r).slice(0, 120)}`,
      );
      return json(res, r.ok ? 200 : 400, r);
    }
