...
function tail(path, bytes = TAIL_BYTES) {
  ...
  try { const buf = Buffer.allocUnsafe(len); readSync(fd, buf, 0, len, start); return buf.toString('utf8') }
...
function transcriptFiles() {
  const out = []
  let projDirs = []
  try { projDirs = readdirSync(PROJECTS) } catch { return out }
...
const TERMINAL_APPS = new Set(['Terminal', 'iTerm2', 'iTerm', 'Ghostty', 'WezTerm', 'kitty', 'Alacritty', 'Warp', 'Hyper'])
...
async function runningProcs() {
  const procs = []
  let pids = []
  try { const { stdout } = await execFileP('/bin/sh', ['-c', 'pgrep -x claude']); pids = stdout.split('\n').map((s) => s.trim()).filter(Boolean) }
  catch { return procs }
  for (const pid of pids) {
    let cwd = null, tty = null
    try { const { stdout } = await execFileP('/usr/sbin/lsof', ['-a', '-p', pid, '-d', 'cwd', '-Fn']); cwd = stdout.split('\n').find((l) => l.startsWith('n'))?.slice(1) || null } catch { /* ignore */ }
    try { const { stdout } = await execFileP('/bin/ps', ['-o', 'tty=', '-p', pid]); const t = stdout.trim(); if (t && t !== '??') tty = t } catch { /* ignore */ }
    if (!cwd) continue
...
export async function snapshotSessions({ limit = 8 } = {}) {
  ...
  for (const f of files) {
    ...
    if (!prev || f.mtime > prev.mtime) byProject.set(key, { ...parsed, mtime: f.mtime, project: key })
  }
  ...
  const sessions = [...byProject.values()]
    .map((s) => {
      ...
      return { ...s, running: !!proc, host: proc?.host || null, tty: proc?.tty || null, app: proc?.app || null }
    })
...
function readSession(f) {
  let text
  try { text = tail(f.path) } catch { return null }
  const lines = text.split('\n').filter(Boolean)
  // The tail may start mid-line; drop the first (likely partial) line.
  if (lines.length > 1) lines.shift()
  let cwd = null, sessionId = null, lastUser = null, lastAssistant = null, lastTs = null
