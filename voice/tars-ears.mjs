...
const TARGET = process.env.TARS_EARS_TARGET === 'inject'
...
function stopSpeaking(said) {
  ...
  fetch(SPEAK_STOP_URL, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ scope: 'all' }), signal: AbortSignal.timeout(2000) }).catch((e) => log('stop failed:', e.message))
}
...
const WAKE_WORDS = new Set(['tars', 'tar', 'tarz', 'tarss', 'taurus', 'tauris', 'torres', 'tarus', 'taras', 'tara', 'taris', 'terrace', 'tears', 'thars', 'tarsh', 'tarr', 'dars', 'taars', 'doris', 'doras', 'toris', 'dora', 'tarris', 'boris', 'tauris', 'thors', 'tors'])
...
function cleanLine(raw) {
  ...
  s = s.replace(/\[[^\]]*\]/g, '').replace(/\([^)]*\)/g, '').trim()   // inline markers
  return s
}
...
async function forward(text, addressed) {
  try {
    const res = await fetch(TARGET.url, {
      ...
      body: JSON.stringify(TARGET.body(text, addressed)), signal: AbortSignal.timeout(35000),
    })
    log(res.ok ? `-> sent (addressed=${addressed}): ${JSON.stringify(text)}` : `target http ${res.status}`)
...
const wnorm = (s) => s.toLowerCase().replace(/[^a-z0-9 ]/g, '').replace(/\s+/g, ' ').trim()
...
function stitch(buf, seg) {
  ...
  const bl = wnorm(buf).split(' '), sl = wnorm(s).split(' '), sw = s.split(/\s+/)
  const maxK = Math.min(bl.length, sl.length, 15)
  let best = 0
  for (let k = maxK; k >= 1; k--) {
    let ok = true
    for (let i = 0; i < k; i++) if (bl[bl.length - k + i] !== sl[i]) { ok = false; break }
    if (ok) { best = k; break }
  }
...
function flushTurn() {
  ...
  turn = null
  if (text.length < MIN_CHARS) { log('turn empty, nothing sent'); return }
...
function handleUtterance(text) {
  ...
  if (speaking && STOP_RE.test(text)) { stopSpeaking(text); turn = null; return }
  if (speaking) { log('drop (TARS speaking)'); return }
  ...
  if (flag(AWAITING_CONFIRM)) { log(`confirm answer: ${JSON.stringify(text)}`); forward(text, true); return }

  // Mid-turn: every segment is a continuation of the addressed command — no wake needed.
  if (turn) { turn.text = stitch(turn.text, text); turn.segs++; scheduleFlush(); return }
  ...
  if (open) { forward(text, false); return }        // open-mic ambient (opt-in), answer-only
  log(`drop (no wake): ${JSON.stringify(text)}`)     // wake-gated default: ignore ambient
}

function start() {
  if (!existsSync(MODEL)) { log('FATAL: model not found at', MODEL); process.exit(1) }
  ...
  const args = [
    '-m', MODEL, '-t', THREADS, '--step', '0', '--length', LENGTH,
    '-vth', VAD_THOLD, '-c', CAPTURE, '-l', LANG, '-nf',
  ]
  ...
  w.on('exit', (code) => { log(`whisper-stream exited (${code}); restarting in 2s`); setTimeout(start, 2000) })
  w.on('error', (e) => { log('whisper-stream error:', e.message) })
}

log(`ears up · model=${MODEL} · wake-gated (open-mic: ${flag(EARS_OPEN)}) · enabled: ${flag(EARS_ON)}`)
start()
