...
>
> **Shortcut:** `tars init` does steps 1–2 (server + persona, plus registering the MCP) without
> ever blocking on a prompt, and prints a JSON summary you can parse before picking up at
> step 3. Pass the profile explicitly so setup never stops to ask:
>
> ```bash
> curl -fsSL https://raw.githubusercontent.com/fonsecabc/tars/main/install.sh | bash -s -- \
>   --all --profile simple --owner-name "<their name>" --install-prompt --yes --json
> ```
>
