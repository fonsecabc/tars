...
reach it, tailnet-only is the _only_ safe exposure). Postgres and Ollama have no host
...
2. **Add Server**:
   ...
   - **Cloud config**: paste the contents of [`cloud-init.yaml`](cloud-init.yaml) _after_
...
On the **server**, restore into Postgres _before_ the app has written anything:
...
- **Encrypted, off-box, off-provider, tamper-resistant backups.** restic encrypts
  ...
  so a box compromise can't read _or_ silently destroy your backup history. Losing the whole
